...
import { View, Pressable, ScrollView, Modal, Platform, TextInput, Image, Alert } from 'react-native';
...
export default function FoldersScreen() {
  ...
  const { folders, notes, createFolder, renameFolder, deleteFolder, createNote } = useNotesStore();
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={['top', 'left', 'right']}>
      ...
      <View className="flex-row items-center justify-between px-6 py-4">
        <AppText weight="bold" className="text-3xl">Folders</AppText>
        <Pressable
        ...
        >
          {user?.avatarUrl ? (
            ...
            <AppText weight="semibold" className="text-[#e4b022] dark:text-[#d4af37]">
              {user?.displayName ? user.displayName.split(' ').map(n => n[0]).join('').toUpperCase() : 'FP'}
            </AppText>
          )}
        </Pressable>
      </View>
      ...
      <ScrollView className="flex-1 px-6">
        ...
        <View className="mt-4 bg-secondary/20 dark:bg-secondary/10 rounded-2xl border border-border overflow-hidden">
          {/* All Notes Row */}
          <Pressable
            onPress={() => router.push({ pathname: '/folder/[id]', params: { id: 'all' } } as any)}
            className="flex-row items-center justify-between px-4 py-4 border-b border-border active:bg-secondary/40"
          >
            <View className="flex-row items-center">
              <Ionicons name="documents-outline" size={22} className="text-[#e4b022] dark:text-[#d4af37] mr-3" color={Platform.OS === 'ios' ? '#e4b022' : '#d4af37'} />
              <AppText weight="medium" className="text-base text-foreground">All Notes</AppText>
            </View>
            <View className="flex-row items-center">
              <AppText className="text-muted-foreground mr-1.5">{totalNotesCount}</AppText>
              <Ionicons name="chevron-forward" size={16} className="text-muted-foreground/60" color="hsl(var(--muted-foreground))" />
            </View>
          </Pressable>

          {/* Uncategorized Row */}
          <Pressable
            onPress={() => router.push({ pathname: '/folder/[id]', params: { id: 'uncategorized' } } as any)}
            className="flex-row items-center justify-between px-4 py-4 border-b border-border active:bg-secondary/40"
          >
            <View className="flex-row items-center">
              <Ionicons name="folder-open-outline" size={22} className="text-[#e4b022] dark:text-[#d4af37] mr-3" color={Platform.OS === 'ios' ? '#e4b022' : '#d4af37'} />
              <AppText weight="medium" className="text-base text-foreground">Uncategorized</AppText>
            </View>
            <View className="flex-row items-center">
              <AppText className="text-muted-foreground mr-1.5">{uncategorizedCount}</AppText>
              <Ionicons name="chevron-forward" size={16} className="text-muted-foreground/60" color="hsl(var(--muted-foreground))" />
            </View>
          </Pressable>
          ...
          {/* Dynamic Folders */}
          {folders.map((folder) => (
            <View key={folder.id} className="border-b border-border flex-row items-center">
              <Pressable
                onPress={() => router.push({ pathname: '/folder/[id]', params: { id: folder.id } } as any)}
                className="flex-1 flex-row items-center justify-between px-4 py-4 active:bg-secondary/40"
              >
                <View className="flex-row items-center">
                  <Ionicons name="folder-outline" size={22} className="text-[#e4b022] dark:text-[#d4af37] mr-3" color={Platform.OS === 'ios' ? '#e4b022' : '#d4af37'} />
                  <AppText weight="medium" className="text-base text-foreground">{folder.name}</AppText>
                </View>
                <View className="flex-row items-center">
                  <AppText className="text-muted-foreground mr-1.5">{getNoteCount(folder.id)}</AppText>
                  <Ionicons name="chevron-forward" size={16} className="text-muted-foreground/60" color="hsl(var(--muted-foreground))" />
                </View>
              </Pressable>
              ...
              <View className="flex-row pr-3 gap-x-2">
                ...
                >
                  <Ionicons name="pencil-outline" size={16} className="text-[#e4b022] dark:text-[#d4af37]" color={Platform.OS === 'ios' ? '#e4b022' : '#d4af37'} />
                </Pressable>
                <Pressable
                  onPress={() => {
                    if (Platform.OS === 'web') {
                  ...
                  className="p-1 active:opacity-60"
                >
                  <Ionicons name="trash-outline" size={16} className="text-destructive" color="red" />
                </Pressable>
              </View>
            </View>
          ))}
        ...
        <View className="mt-8 mb-6 flex-row items-center justify-center p-3 rounded-xl bg-[#e4b022]/5 dark:bg-[#d4af37]/5 border border-[#e4b022]/10 dark:border-[#d4af37]/10">
          ...
          <AppText weight="medium" className="text-xs text-[#e4b022] dark:text-[#d4af37]">
      ...
      >
        ...
        >
          <Ionicons name="folder-open-outline" size={24} className="text-[#e4b022] dark:text-[#d4af37]" color={Platform.OS === 'ios' ? '#e4b022' : '#d4af37'} />
        </Pressable>
        ...
        >
          <Ionicons name="create-outline" size={24} className="text-[#e4b022] dark:text-[#d4af37]" color={Platform.OS === 'ios' ? '#e4b022' : '#d4af37'} />
        </Pressable>
      </View>
      ...
      >
        <View className="flex-1 bg-black/60 items-center justify-center px-6">
          <View className="w-full bg-card rounded-2xl p-5 border border-border">
            ...
            {validationError ? (
              <AppText className="text-xs text-destructive mb-3">{validationError}</AppText>
            ) : null}
